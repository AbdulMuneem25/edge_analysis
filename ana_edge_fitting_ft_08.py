...
def make_graphs(tracks, output_dir, title):
    vals, bin_edges = np.histogram(tracks, 100,(1,5))
    #print(vals)
...
if __name__ == '__main__':
    ...
    list_cordinate_x = np.random.uniform(0,5,30000)
    ...
    bin_heights, bin_edges = np.histogram(list_cordinate_x_err, 100,(0,5))
    #print(vals)
    bin_centers = 0.5*(bin_edges[1:] + bin_edges[:-1])
    plt.errorbar(bin_centers, bin_heights, np.sqrt(bin_heights),  4.0/20.0,fmt='o', ecolor='g', color='blue')
    
    popt, _ = curve_fit(gaussian, bin_centers, bin_heights, p0=[1., 1., 1.])
    print(popt)
    x_interval_for_fit = np.linspace(bin_edges[0], bin_edges[-1], 30000)
    ...
    plt.xlabel('data_ran_num')
    plt.ylabel('Counts')
    plt.savefig(f"{output_dir}/fitting_ran_data_sxlim.png")
    plt.show()
    plt.clf()
    ...
    # a = 0 
    # b = 1
    # fx = gaussian(x_interval_for_fit, mu, 30000, sigma)
